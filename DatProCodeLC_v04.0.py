...
def PeakFinder(dat,tr_hd,tr_dis):
      print('threshold (y)= ',tr_hd,'[N]')
      print('threshold (y)= ',scaleUp_std(tr_hd,3),'[kN/m]')
...
def scaleUp_std(df,ttype):
      ...
      if ttype==2:
            ...
            data=df['data']*scale_fac*scale_fac*scale_fac/1000/(rppl/1000*scale_fac)   #in kN/m
            print("------------------------sacled data preview------------------------------- ")
            print ("Length of the riserporch connected = " ,rppl)
            print ('Scale factor = ',scale_fac)
            print('density of water = ',1025 ,'kg/m^3')
            print ('acceleration due to gravity =',9.81 ,'m/s^2')
            print(data.head())
      ...
      if ttype==3:
            ...
            scale_fac=90
              #in bar
            return(df*scale_fac*scale_fac*scale_fac/1000/(rppl/1000*scale_fac))   #in kN/m
            

def MeanNPeak(df,datum) :
      dat=scaleUp_std(df,2)

      #find the mean and peak of the given data series 
      import statistics as stat
      sort_dat=dat.sort_values(ascending=False) 

      SignOfPeaks=stat.mean(sort_dat[0:round(sort_dat.size/3)]) - scaleUp_std(datum,3) # mean of the data 
      ...
      print('maximum of peak              :',MaxVal,'kN/m')
      print('avarage of the peaks         :',(stat.mean(dat)-scaleUp_std(datum,3)),'kN/m')
      print('significant of peaks         :',SignOfPeaks,'kN/m')
...
#      
      ...
      fl_name=["Results/Ballast/B_240/S8.ASC"]

      fl_out='Results/Ballast/B_240/LC/S8.out'
...
#      fl_name=['QX/S1.txt','QX/S2.txt','QX/S3.txt','QX/S4.txt','QX/S5.txt','QX/S6.txt','QX/S7.txt','QX/S8.txt'] 
      ...
      for j in fl_name:
            for i in pt_name:
                  ...
                  plt.xlabel('index')
                  plt.ylabel('Force[N]')
                  plt.title('Force record time series')
                  plt.grid(True)
